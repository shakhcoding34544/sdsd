...
def init_db():
    ...
    # Add instagram_shown column if it doesn't exist (for migration)
    try:
        c.execute('ALTER TABLE users ADD COLUMN instagram_shown INTEGER DEFAULT 0')
...
def get_channel_link(channel_username):
    if channel_username is None:
        return None
    url = str(channel_username).strip()
    if not url:
        return None
    if url.startswith('@'):
...
def check_subscription(user_id):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT instagram_shown FROM users WHERE user_id = ?', (user_id,))
    result = c.fetchone()
    instagram_shown = result[0] if result else 0
    conn.close()
    
    unsubscribed = []
    instagram_channel = None
    for idx, channel in enumerate(channels, start=1):
        # channels are (channel_id, channel_username, is_private)
        chan_id, chan_username, is_private = channel

        # Detect Instagram channel entry
        if chan_username and ('instagram' in str(chan_username).lower() or str(chan_username).lower().startswith('https://instagram.com')):
            instagram_channel = (chan_id, chan_username, is_private)
            # Do not treat Instagram as required; it will be shown optionally once
            continue

        # If neither id nor username is available, skip
        if not chan_id and not chan_username:
            continue

        # For public channels try to check membership via get_chat_member when we have an id
        if not is_private:
            try:
                target = chan_id if chan_id else chan_username
                status = bot.get_chat_member(target, user_id).status
                if status not in ['member', 'administrator', 'creator']:
                    unsubscribed.append((chan_id, chan_username, is_private))
            except Exception as e:
                # If we can't check (maybe username/id invalid), consider user unsubscribed
                msg = str(e).lower()
                if 'member list is inaccessible' in msg:
                    print(f"Kanal tekshirishda xatolik: member list inaccessible for {chan_username or chan_id}")
                else:
                    print(f"Kanal tekshirishda xatolik: {e}")
                unsubscribed.append((chan_id, chan_username, is_private))
        else:
            # Private channel: require user action (open link / invite)
            unsubscribed.append((chan_id, chan_username, is_private))

    return len(unsubscribed) == 0, unsubscribed, instagram_channel
def subscription_keyboard(channels_to_show=None, user_id=None, include_instagram=True):
    ...
    # Build a display list preserving the active channels order.
    # We'll show buttons for channels the user is unsubscribed from (channels_to_show)
    # and also include the Instagram channel (if present and not shown yet) without
    # marking it as optional — numbering will reflect its position among displayed buttons.
    display_list = []
    try:
        active = get_active_channels()
        # Build a set of identifiers for unsubscribed channels for quick match
        unsub_ids = set()
        if channels_to_show:
            for ch in channels_to_show:
                cid, cun, cip = ch
                key = str(cid) if cid else str(cun)
                unsub_ids.add(key)

        # Determine if instagram should be shown
        show_instagram = False
        inst_key = None
        if user_id and include_instagram:
            try:
                conn = get_db()
                c = conn.cursor()
                c.execute('SELECT channel_id, channel_username, is_private FROM channels WHERE is_active = 1 AND (lower(channel_username) LIKE ? OR lower(channel_username) LIKE ?) LIMIT 1', ('%instagram%', 'https://instagram.com%'))
                inst = c.fetchone()
                if inst:
                    c.execute('SELECT instagram_shown FROM users WHERE user_id = ?', (user_id,))
                    res = c.fetchone()
                    shown = res[0] if res else 0
                    if not shown:
                        show_instagram = True
                        inst_key = str(inst[0]) if inst[0] else str(inst[1])
            except Exception:
                pass
            finally:
                try:
        ...
        for ch in active:
            cid, cun, cip = ch
            key = str(cid) if cid else str(cun)
            if key in unsub_ids or (show_instagram and key == inst_key):
                display_list.append((cid, cun, cip))
    except Exception:
        # Fallback: show given channels_to_show in provided order
        display_list = channels if channels_to_show is None else channels_to_show

    for idx, channel in enumerate(display_list, start=1):
        chan_id, chan_username, is_private = channel
        channel_name = f"{idx}-kanal"
        # Prefer saved invite link (channel_username may store invite link for private channels)
        link = get_channel_link(chan_username) or (f"https://t.me/{abs(chan_id)}" if chan_id else None)
        if link:
...
def join_private_channel(call):
    ...
    try:
        ...
        return
    # We do not send join requests from the bot. Provide the channel link so the user
    # can open it and, if it's private, Telegram will show the join-request (zayafka) UI.
    channel_url = f"https://t.me/{abs(channel_id)}" if str(channel_id).startswith('-100') else f"https://t.me/{channel_id}"
    try:
        bot.answer_callback_query(call.id, '⚠️ Iltimos, kanalga o\'ting va join/zayafka yuboring. Bot zayafka qabul qilmaydi.', show_alert=True)
    except Exception:
        pass
    try:
        bot.send_message(call.from_user.id, f"Kanalga o'tish: {channel_url}")
    except Exception:
        pass
...
def check_sub_decorator(func):
    ...
    def wrapper(msg):
        ...
        if is_banned(user_id):
            ...
            return
        subscribed, unsubscribed_channels, instagram_channel = check_subscription(user_id)
        if not subscribed:
            required_count = len(unsubscribed_channels)
            msg_text = (
                f"🚀 Botdan to'liq foydalanish uchun kanallarga obuna bo'ling:\n\n"
                f"Obuna bo'lib, /start bosib qayta urinib ko'ring."
            )
            bot.send_message(
                msg.chat.id,
                msg_text,
                reply_markup=subscription_keyboard(unsubscribed_channels, user_id=user_id, include_instagram=True)
            )
            return
        return func(msg)
    return wrapper
...
def check_sub_callback(call):
    ...
    if subscribed:
        conn = get_db()
        c = conn.cursor()
        c.execute('SELECT 1 FROM channels WHERE is_active = 1 AND (channel_username LIKE ? OR channel_username LIKE ?)', ('%instagram%', 'https://instagram.com%'))
        has_instagram = c.fetchone()
        if has_instagram:
            c.execute('SELECT instagram_shown FROM users WHERE user_id = ?', (user_id,))
            result = c.fetchone()
            if result and result[0] == 0:
                c.execute('UPDATE users SET instagram_shown = 1 WHERE user_id = ?', (user_id,))
                conn.commit()
        conn.close()
        ...
        try:
            ...
            # Ignore 'message is not modified' errors from Telegram API
            msg = str(e).lower()
        ...
        text = "❌ Obuna bo'lmagan kanallaringiz:\n\n⬇️ Pasdagi tugmalardan foydalanib obuna bo'ling."
        try:
            bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=subscription_keyboard(unsubscribed_channels, user_id=user_id, include_instagram=False))
        except Exception as e:
            # Ignore 'message is not modified' errors from Telegram API
            msg = str(e).lower()
            if 'message is not modified' in msg:
...
def start(msg):
    ...
    conn = get_db()
    c = conn.cursor()
    c.execute('''INSERT OR IGNORE INTO users VALUES (?, ?, ?, ?, ?, 0, 0)''',
...
def save_new_serial(msg, title, description):
    ...
    # Check if code already exists in movies or serials
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT code FROM movies WHERE code = ?', (code,))
    if c.fetchone():
        conn.close()
        bot.send_message(msg.chat.id, f"⚠️ {code} kodi allaqachon kinoda mavjud!")
        bot.register_next_step_handler(msg, lambda m: save_new_serial(m, title, description))
        return
    ...
    if c.fetchone():
        conn.close()
        bot.send_message(msg.chat.id, f"⚠️ {code} kodi allaqachon serialda mavjud!")
        bot.register_next_step_handler(msg, lambda m: save_new_serial(m, title, description))
        return
...
def channels_menu(msg):
    if not is_admin(msg.from_user.id):
        return
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row("➕ Kanal qo'shish", "📷 Instagram account qo'shish")
    markup.row("🗑 Kanal o'chirish", "🔄 Kanal o'chirish/yoqish")
    markup.row("🔙 Orqaga")
...
@bot.message_handler(func=lambda m: m.text == "📷 Instagram account qo'shish")
@check_sub_decorator
def add_instagram_start(msg):
    if not is_admin(msg.from_user.id):
        return
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row("🔙 Orqaga")
    bot.send_message(msg.chat.id, "📷 Instagram username yoki linkini kiriting (masalan: @username yoki https://instagram.com/username):", reply_markup=markup)
    bot.register_next_step_handler(msg, save_instagram)

def save_instagram(msg):
    if msg.text == "🔙 Orqaga":
        channels_menu(msg)
        return
    username = msg.text.strip()
    if not username:
        bot.send_message(msg.chat.id, "❌ Username bo'sh bo'lishi mumkin emas!")
        return
    # Save as channel
    conn = get_db()
    c = conn.cursor()
    c.execute('INSERT INTO channels (channel_username, added_by, added_date, is_active, is_private) VALUES (?, ?, ?, 1, 0)',
              (username, msg.from_user.id, datetime.now().strftime("%Y-%m-%d %H:%M")))
    conn.commit()
    conn.close()
    bot.send_message(msg.chat.id, f"✅ Instagram account '{username}' qo'shildi!")
...
def remove_channel_start(msg):
    ...
    for idx, ch in enumerate(channels, 1):
        # ch = (row_id, channel_username, channel_id)
        row_id, chan_username, chan_id = ch
        display = None
        if chan_username:
            display = str(chan_username)
        elif chan_id:
            display = str(chan_id)
        else:
            display = f"ID:{row_id}"
    ...
    markup.row("🔙 Orqaga")
    bot.send_message(msg.chat.id, text, parse_mode="Markdown", reply_markup=markup)
    bot.register_next_step_handler(msg, lambda m, ch_list=channels: remove_channel_id(m, ch_list))
def remove_channel_id(msg, channels=None):
    if msg.text == "🔙 Orqaga":
        channels_menu(msg)
        return
    try:
        channel_idx = int(msg.text.strip()) - 1
    except:
        bot.send_message(msg.chat.id, "❌ Raqam kiriting!")
        return
    
    if channels is None or channel_idx < 0 or channel_idx >= len(channels):
        bot.send_message(msg.chat.id, "❌ Kanal topilmadi!")
        return
    
    row_id = channels[channel_idx][0]
    
    conn = get_db()
    c = conn.cursor()
    # Get channel_id to delete related records
    c.execute('SELECT channel_id FROM channels WHERE id = ?', (row_id,))
    result = c.fetchone()
    if not result:
        conn.close()
        bot.send_message(msg.chat.id, "❌ Kanal topilmadi!")
        return
    channel_id = result[0]
    # Delete from join_requests table (for private channels)
    if channel_id:
        c.execute('DELETE FROM join_requests WHERE channel_id = ?', (channel_id,))
    ...
    conn.close()
    if deleted > 0:
        bot.send_message(msg.chat.id, f"✅ Kanal to'liq o'chirildi!")
    else:
        bot.send_message(msg.chat.id, "❌ Kanal topilmadi!")
...
def toggle_channel_start(msg):
    ...
    text = "📋 Kanallar:\n\n"
    for idx, ch in enumerate(channels, 1):
        # ch = (row_id, channel_username, is_active)
        row_id, chan_username, is_active = ch
        status = "✅ Faol" if is_active == 1 else "❌ O'chiq"
        display = chan_username if chan_username else f"ID:{row_id}"
        text += f"{idx}. {display} - {status}\n"
    text += "\n🔢 O'zgartirish uchun kanal raqamini kiriting:"
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.row("🔙 Orqaga")
    bot.send_message(msg.chat.id, text, reply_markup=markup)
    bot.register_next_step_handler(msg, lambda m, ch_list=channels: toggle_channel_id(m, ch_list))
def toggle_channel_id(msg, channels=None):
    if msg.text == "🔙 Orqaga":
        channels_menu(msg)
        return
    try:
...
def add_movie_code(msg):
    ...
    # Avval kinolarda bormi tekshiramiz
    c.execute('SELECT * FROM movies WHERE code = ?', (code,))
    if c.fetchone():
        conn.close()
        bot.send_message(msg.chat.id, f"⚠️ {code} kodi allaqachon kinoda mavjud!")
    ...
    # Endi seriallarda ham bormi tekshiramiz
    c.execute('SELECT * FROM serials WHERE code = ?', (code,))
    if c.fetchone():
        conn.close()
        bot.send_message(msg.chat.id, f"⚠️ {code} kodi allaqachon serialda mavjud!")
...
def catch_all_text(msg):
    ...
    if text == "🔙 Orqaga":
        back_handler(msg)
        return
    admin_commands = ["👑 Admin Panel", "📢 Kanallar", "🗑 Kanal o'chirish", "📷 Instagram account qo'shish", "🔄 Kanal o'chirish/yoqish", "📋 Kanallar ro'yxati", "➕ Kino qo'shish", "✏️ Tahrirlash", "🗑 O'chirish", "🚫 Ban/Unban", "📊 Statistika", "📣 Reklama", "📋 Adminlar ro'yxati", "💾 Zaxira", "➕ Admin qo'shish"]
    if text in admin_commands:
        bot.send_message(msg.chat.id, "❌ Siz admin emassiz! Faqat adminlar uchun.")
        return
